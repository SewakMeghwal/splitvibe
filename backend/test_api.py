"""
Automated Verification Suite for SplitVibe Backend (Clean Data & Uploads)
Tests File Uploads, Squad Creation, Auth Register/Login, REST APIs, Debt Graph, Settings, OCR, FX.
"""

import urllib.request
import json
import sys

BASE_URL = "http://localhost:8000/api"

def test_endpoint(name, url, method="GET", payload=None, expected_status=200):
    try:
        req = urllib.request.Request(url, method=method)
        if payload:
            data = json.dumps(payload).encode('utf-8')
            req.add_header('Content-Type', 'application/json')
        else:
            data = None
            
        with urllib.request.urlopen(req, data=data) as response:
            status = response.status
            body = json.loads(response.read().decode('utf-8'))
            if status == expected_status:
                print(f"[PASS] {name} (HTTP {status})")
                return body
            else:
                print(f"[FAIL] {name} - Expected {expected_status}, got {status}")
                sys.exit(1)
    except urllib.error.HTTPError as e:
        if e.code == expected_status:
            print(f"[PASS] {name} (Expected HTTP {e.code})")
            return json.loads(e.read().decode('utf-8'))
        else:
            print(f"[FAIL] {name} - HTTP Error {e.code}: {e.reason}")
            sys.exit(1)
    except Exception as e:
        print(f"[FAIL] {name} - Error: {e}")
        sys.exit(1)

def run_all_tests():
    print("--- Starting SplitVibe Verification Tests ---")

    # 1. Auth Register Test
    new_handle = f"@user_{hash(BASE_URL) % 10000}"
    reg_res = test_endpoint("Register New Account", f"{BASE_URL}/auth/register", method="POST", payload={
        "name": "Alex Rivera",
        "handle": new_handle,
        "avatar": "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150",
        "bio": "Backpacker & squad creator",
        "password": "secretpassword123"
    })
    assert reg_res.get("status") == "success"
    uid = reg_res["user"]["id"]

    # 2. Create Squad API
    squad_res = test_endpoint("Create New Squad", f"{BASE_URL}/squads", method="POST", payload={
        "name": "Tahoe Ski Roadtrip 🏔️",
        "description": "Weekend trip to Lake Tahoe",
        "avatar": "https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?w=300",
        "category": "Travel",
        "member_ids": [uid]
    })
    assert squad_res.get("status") == "created"
    squad_id = squad_res["squad_id"]

    # 3. Squad Summary & Debt Graph
    summary = test_endpoint("Get Squad Summary & Debt Graph", f"{BASE_URL}/squads/{squad_id}/summary")
    assert "simplified_debts" in summary

    # 4. OCR & FX APIs
    ocr_res = test_endpoint("Scan Receipt (OCR)", f"{BASE_URL}/ocr/scan-receipt", method="POST")
    assert "items" in ocr_res and "total" in ocr_res

    fx_rates = test_endpoint("Get FX Exchange Rates", f"{BASE_URL}/fx/rates")
    assert "EUR" in fx_rates

    print("\n--- ALL VERIFICATION TESTS PASSED SUCCESSFULLY! ---")

if __name__ == "__main__":
    run_all_tests()
